...
(function () {
  ...
  function showTalk() {
    ...
    el('continue').textContent = talk.at === talk.lines.length - 1 ? (talk.choices?'Choose a place':'Back to the café') : 'Continue';
  ...
  el('action').onclick = function () { act(); };
  ...
  function update(dt) {
    time+=dt;player.animT=patron.animT=time;zoom+=((talk?1.95:1.45)-zoom)*Math.min(1,dt*5);
    ...
    if(working){player.pose='stand';working.elapsed+=dt;if(working.elapsed>=working.task.duration){var t=working.task;working=null;finishWork(t);}return;}
  ...
  function room(){rect(0,0,960,600,'#201e24');rect(194,122,572,398,'#15191a');rect(208,132,544,178,has('walls')?'#a4967b':'#736f62');
    ...
    if(working){var wx=player.x+16+Math.sin(time*15)*3,id=working.task.id;
  ...
  function cameraFocus(){return Math.max(0,Math.min(1,(zoom-1.45)/0.5));}
  function render(){
    g.save();var focus=cameraFocus();g.translate(480,290-45*focus);g.scale(zoom,zoom);g.translate(-480,-320-55*focus);room();g.restore();
    ...
    uiText('action',working?working.task.verb+'…':n?n.label:'Take a moment');
    el('action').disabled=busy||!!(n&&['talk','chat','order','deliver','rest'].indexOf(n.id)>=0&&!arrived);
    uiText('status',saveFailed?'Your browser cannot save this visit. You can still keep playing.':notice||'Lunafreya · “I think there is a café in here somewhere.”');
  ...
  canvas.addEventListener('pointerdown',function(e){if(talk||working||modal)return;var r=canvas.getBoundingClientRect(),x=(e.clientX-r.left)*960/r.width,y=(e.clientY-r.top)*600/r.height,n=next(),focus=cameraFocus();x=(x-480)/zoom+480;y=(y-290+45*focus)/zoom+320+55*focus;var invitation=FLEUR.invitation(save);if(invitation&&Math.hypot(x-patron.x,y-(patron.y-35))<32){el('invitation').click();return;}if(n&&Math.hypot(x-n.x,y-(n.y-35))<40){act();return;}x=Math.max(L.bounds.left,Math.min(L.bounds.right,x));y=Math.max(L.bounds.top,Math.min(L.bounds.bottom,y));if(!blocked(x,y)){resting=false;target={x:x,y:y};}});
  ...
  if(new URLSearchParams(location.search).has('dev'))window.__game={state:save,player:player,patron:patron,layout:L,update:update,act:act,
    inspect:function(){return {working:working&&working.task.id,talking:!!talk,modal:modal,arrived:arrived,resting:resting,invitation:FLEUR.invitation(save)};},
