...
(function () {
  ...
  try {
    ...
    check(run(w, function () { return !w.patrons.length && w.waterfront.tables.every(function (t) { return !t.dirty && !t.cleaning; }) && w.barista.state === 'idle'; }, 260, function () {
      ...
      if (w.barista.outside) check(SIM.entityDrawables(w).draws.length === 1 + SIM.visitorActors(w).filter(a=>!a.social).length + w.patrons.filter(function (p) { return !p.outside; }).length, 'Lunafreya duplicated indoors while outside');
