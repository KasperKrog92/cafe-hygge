...
$testSession = 'hygge-ship-' + [guid]::NewGuid().ToString('N').Substring(0, 8)
$repo = Split-Path $PSScriptRoot -Parent
$output = Join-Path $repo '.art-review/ship-motion'
...
try {
    ...
    $code = Get-Content -Raw (Join-Path $PSScriptRoot 'verify-ship.js')
    $result = agent-browser --session $testSession eval $code
    $verified = $LASTEXITCODE -eq 0
    $payload = agent-browser --session $testSession eval 'window.shipFrames'
    ...
    if (!$verified) { throw 'Ship verification failed; inspect the exported frames' }
