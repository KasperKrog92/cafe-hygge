...
(function () {
  const L = SCENE.L, R = SIM._, w = __dev.furnishedWorld({}), failures = [];
  ...
  stops.forEach((a, ai) => stops.forEach((b, bi) => {
    ...
    const t = performance.now(); SIM.withWorld(w,()=>R.makePath(e,b.x,b.y)); worstMs = Math.max(worstMs,performance.now()-t);
    ...
    let arrived = false;
    for(let n=0;n<200;n++) { if(SIM.withWorld(w,()=>R.walker(e,.25))) { arrived=true; break; } }
  ...
  const direct = {x:L.orderSpot.x,y:L.orderSpot.y}; SIM.withWorld(w,()=>R.makePath(direct,L.pickupSpot.x,L.pickupSpot.y));
  ...
  for(const reverse of [false,true]) {
    ...
    const e={x:a.x,y:a.y,speed:60};SIM.withWorld(w,()=>R.makePath(e,b.x,b.y));
    for(let n=0;n<1000;n++) {
      const arrived=SIM.withWorld(w,()=>R.walker(e,.05));
